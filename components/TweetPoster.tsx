 ...
 * - Height follows content (no forced 4:5 floor) so short tweets don't leave a
 *   large empty band above the brand footer.
 * - Horizontal padding is `px-10` (40px). `TweetMedia` opts out via `-mx-10`
 *   so images bleed edge-to-edge; text/header/quote/stats keep the inset.
 */
export default function TweetPoster({ data }: { data: TweetData }) {
  return (
    <div
      id="poster"
      className="flex w-[1080px] flex-col bg-[radial-gradient(circle_at_top_right,rgba(29,155,240,0.06),transparent_40%),radial-gradient(circle_at_bottom_left,rgba(168,107,255,0.05),transparent_40%),#0a0a0f] px-10 py-12 text-zinc-100"
    >
