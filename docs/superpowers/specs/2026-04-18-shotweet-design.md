...
- Renders `<TweetPoster data={...} />` using the locked visual style:
  ...
  - Media layout: **all images are stacked vertically at full poster width (1080px)** with each image's natural aspect ratio preserved. The same single-column treatment applies to 1, 2, 3, or 4 images — chosen over X's mosaic grids because the poster is a tall mobile-friendly canvas where a single column reads more naturally and avoids cropping faces. **Media bleeds edge-to-edge** via `-mx-10` on the wrapper (cancelling the parent `px-10`), and the previous `border` + `rounded-2xl` are dropped because both look like stray strokes once flush with the canvas. Stacked images keep a thin `gap-1` divider via the dark canvas peeking through.
  ...
  - Stats row above a top border: `<n> replies · <n> reposts · <n> likes · <n> views`, with a subtle **X mark** (SVG, muted gray, ~16px) aligned to the **right end of the same row** (no separate overlay layer; included in the PNG screenshot). **Views** are parsed by scanning **all** `[aria-label]` nodes under the tweet for English `… views` / Chinese *次观看*. Logged-out X often puts view count **only** in a **combined** summary string on a wrapper `div`, e.g. `"108 replies, 810 reposts, …, 69672 views"`, with no `data-testid="analytics"`.
